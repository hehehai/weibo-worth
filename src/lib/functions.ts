...
export async function searchUserByKeyword(keyword: string): Promise<any[] | null> {
  try {
    if (!keyword) {
      return [];
    }
    ...
    const userList = (filterResult?.card_group ?? []).map((user: any) => ({
      ...user,
